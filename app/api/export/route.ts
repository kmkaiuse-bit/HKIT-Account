...
export async function GET() {
  try {
    ...
    const rows = applications.map(app => {
      ...
      if (app.claimants) {
        try {
          const rows = JSON.parse(app.claimants) as { name: string; amount: string; description: string }[];
          claimantsText = rows.map(r => `${r.name}: ${r.description} (HKD ${r.amount})`).join('\n');
        } catch {}
      }
      return {
    ...
    ws['!cols'] = [14, 20, 12, 16, 40, 40, 10, 20, 16, 18, 30, 12, 16, 10, 12, 16, 12, 40].map(wch => ({ wch }));
